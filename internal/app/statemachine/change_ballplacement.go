...
func (s *StateMachine) processChangeStartBallPlacement(newState *state.State) (changes []*Change) {
	...
	if noTeamCanPlaceBall(newState) {
		...
		changes = append(changes, CreateCommandChange(state.NewCommand(state.Command_BALL_PLACEMENT, teamInFavor.Opposite())))
		...
		changes = append(changes, CreateCommandChange(state.NewCommand(state.Command_BALL_PLACEMENT, teamInFavor)))
